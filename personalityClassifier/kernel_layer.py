...
class KernelLayer(nn.Module):
    ...
    def __init__(self, 
        ...
        # Connection matrix
        self.W = nn.Parameter(torch.Tensor(self.n_in, self.n_hid).to(self.device))
        # Learnable vectors, used in distance calculation in kernel function
        self.u = nn.Parameter(torch.Tensor(self.n_in, 1, self.n_dim).to(self.device))
        self.v = nn.Parameter(torch.Tensor(1, self.n_hid, self.n_dim).to(self.device))
        # Biases
        self.b = nn.Parameter(torch.Tensor(n_hid).to(self.device))
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        w_hat = self.kernel(self.u, self.v).to(self.device) # [n_in, n_hid]
        ...
        w_eff = (self.W * w_hat).to(self.device) # [n_in, n_hid]
        y = torch.matmul(x, w_eff).to(self.device) + self.b
        y = self.activation(y).to(self.device)
        return y, reg_term
